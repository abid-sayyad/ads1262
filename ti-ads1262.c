...
static int ads1262_init(struct iio_dev *indio_dev)
{
        ...
        /* Setting the ADC to read the internal temperature sensor (Testing purpose)
         * by default it is set to read AIN0 - AIN1 differential channel
         */
        ret = ads1262_reg_write(priv, ADS1262_REG_INPMUX, 0xBA);
        if (ret)
                return ret;
...
static int ads1262_get_samp_freq(struct ads1262_private *priv, int *val)
{
        ...
        /* Extract just the data rate bits using the mask */
 ...
 */
static int ads1262_read(struct ads1262_private *priv, struct
iio_chan_spec const *chan, int *val)
{
        u8 mux_value;
        int ret;

        /* Calculate MUX value based on channel type */
        if (chan->differential) {
                /* For differential channels, set up the MUX with both
channels */
        ...
        /* Wait for conversion to complete - this time depends on the data rate
         * setting in MODE2 register. For simplicity, we use a fixed delay here.
         * In a production driver, this should be calculated based on
the data rate.
         */
        usleep_range(1000, 2000);

        /* Extract the 32-bit value from the response */
        printk("bytes: %d %d %d %d %d %d ", priv->rx_buffer[0],\
priv->rx_buffer[1], priv->rx_buffer[2],
priv->rx_buffer[3],\
priv->rx_buffer[4], priv->rx_buffer[5]);
                ...
                int *val, int *val2, long mask)
{
    ...
    case IIO_CHAN_INFO_RAW:
        switch(chan->type) {
        case IIO_VOLTAGE:
            return ads1262_read(spi, chan, val);

        case IIO_TEMP:
            int ret;
            ret = ads1262_read(spi, chan, val);
            if (ret)
                return ret;
            
            temp = (s64)*val;
            /* Convert to voltage: (raw * 2.5V) / 2^31 */
            temp = (temp * (s64)ADS1262_VOLTAGE_INT_REF_uV) / (1ULL << 31);
            
            /* Convert voltage to temperature in millidegrees */
            temp -= 1500000; /* Subtract 1.5V (0°C voltage) */
            temp = (temp * (s64)MILLI) / ADS1262_TEMP_SENSITIVITY_uV_per_C;
            
            /* Add debug print */
            printk("raw: %d voltage: %lld temp: %lld\n", 
                   *val,
                   (temp + 1500000) / MILLI,  /* Show voltage in mV */
                   temp);
